...
void cmGlobalNixGenerator::WriteNixFile()
{
  ...
  std::cerr << "DEBUG: WriteNixFile() writing to: " << nixFile << std::endl;
  
  cmGeneratedFileStream nixFileStream(nixFile);
  nixFileStream.SetCopyIfDifferent(true);
  
  if (!nixFileStream) {
    std::cerr << "ERROR: Failed to open Nix file for writing: " << nixFile << std::endl;
    return;
  }
  
  std::cerr << "DEBUG: Opened Nix file successfully, starting to write..." << std::endl;

...
void cmGlobalNixMultiGenerator::WriteNixFile()
{
  ...
  nixFileStream << "# Generated by CMake Nix Multi-Config Generator\n";
  nixFileStream << "with import <nixpkgs> {};\n\n";
  nixFileStream << "let\n";
  ...
  const std::string& config)
{
  ...
  if (!compileFlags.empty()) {
    nixFileStream << " " << compileFlags;
  }
  if (!defineFlags.empty()) {
    nixFileStream << " " << defineFlags;
  }
  if (!includeFlags.empty()) {
    nixFileStream << " " << includeFlags;
  }
  
  // Add source file
  std::string relSourcePath = lg->MaybeRelativeToTopBinDir(sourceFile);
  nixFileStream << " \"" << relSourcePath << "\" -o \"$out\"\n";
  nixFileStream << "    '';\n";
  ...
  const std::string& config)
{
  ...
  // Build inputs
  nixFileStream << "    buildInputs = [ " << this->GetCompilerPackage(primaryLang);
  ...
  // Get link flags - we need to compute them here since we don't have a GetLinkFlags method
  ...
  if (targetType == cmStateEnums::EXECUTABLE) {
    ...
    nixFileStream << " $objects";
    if (!linkFlags.empty()) {
      nixFileStream << " " << linkFlags;
    }
    nixFileStream << " -o \"$out\"\n";
  } else if (targetType == cmStateEnums::STATIC_LIBRARY) {
    nixFileStream << "      ar cr \"$out\" $objects\n";
  } else if (targetType == cmStateEnums::SHARED_LIBRARY) {
    if (primaryLang == "CXX") {
      nixFileStream << "      g++";
    } else {
